...
class Mediciones:
    ...
    def fft_componente_principal(self, t, v):
        ...
        v_windowed = v * windows.flattop(
            len(v)
        )  # Utilizo una venta flat-top para mejor mediciones de amplitud

        v_fft = np.fft.fft(v_windowed) / len(v_windowed)  # Calculo y normalizo FFT de la tension
        freq_fft = np.fft.fftfreq(
            len(v_fft), d=t_sample
        )  # Genero lista de frecuencias correspondientes a la FFT
    ...
    def get_cruces_por_cero(self, v, direccion="ambas", delta_min=0.1):
        ...
        # Para encontrar los cruces por cero utilizo hyst() pero al introducir
        # histeresis, la señal de la salida de hyst() no corresponde al momento
        # exacto en donde se cruzo por cero. Para corregir esto, genero
        # "vg_cruce_cero2".
        # Esta es similar a "vg_cruce_cero1" solo que analizada en el sentido
        # opuesto, entonces los cambios de signo los detecta del lado opuesto
        # del cruce por cero real. (Asumiendo que la funcion es aproximadamente
        # lineal en el intervalo donde actua la histeresis)
        # Promediando ambos cruces, se obtiene una mejor estimacion del cruce
        # por cero real.
    ...
    def medir_rc_lissajous(self, t, vg, vr, r, savgol=False, verbose=False):
        ...
        # Promedio los valores de Vr correspondientes, restado con el promedio
        # de los puntos opuestos
