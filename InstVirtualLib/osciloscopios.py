...
class SDS2102(osciloscopio):
    ...
    def get_samplerate(self):
        ...
        sufijo = parte[-1]
        if sufijo in SAMPLE_RATE_DICT:
            valor = parte[:-1] + SAMPLE_RATE_DICT[sufijo]
        else:
            valor = parte  # sin sufijo
    ...
    def get_trdl(self):
        ...
        # Tomar último carácter y ver si es un sufijo conocido
        sufijo = parte[-1]
        if sufijo in TRIGGER_DELAY_DICT:
            valor = parte[:-1] + TRIGGER_DELAY_DICT[sufijo]
        else:
            valor = parte  # sin sufijo
...
class RIGOL_DS2202 (osciloscopio):
    def __init__(self, handler, VXI11):
        super().__init__(handler, VXI11)
